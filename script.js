...
const groundY = 0;             // top surface (grass) is y = 0
const eyeHeight = 120;         // eyes are 120px above feet

// === Player state ===
// posY represents the character "anchor" at feet + characterYOffset
// So feetY = posY - characterYOffset
const characterYOffset = 280;          // distance from posY to model origin
let posX = 0;
let posY = characterYOffset;           // spawn so feet sit exactly on y=0
let posZ = 0;
...
const keys = {};
const speed = 2;
const gravity = 1.5;       // positive increases Y (downwards)
const jumpStrength = 70;   // pixels
let velY = 0;
let grounded = false;
...
function onMouseMove(e) {
  ...
  // Clamp pitch to human-like range
  const maxPitch = 85;
  if (pitch > maxPitch) pitch = maxPitch;
  if (pitch < -maxPitch) pitch = -maxPitch;
  console.log(`view yaw=${yaw.toFixed(1)}°, pitch=${pitch.toFixed(1)}°`);
}
...
function createBlockElement(gx, gy, gz, type, exposedFaces) {
  ...
  const py = gy * BLOCK_SIZE; // groundY = 0, so py is just layer * block size
...
function getTopSurfaceYUnderPlayer() {
  ...
  for (let gy = 0; gy < STONE_LAYERS; gy++) {
    if (worldData.has(keyAt(gx, gy, gz))) {
      return gy * BLOCK_SIZE; // top surface Y in pixels
    }
  }
  return undefined;
}
...
function updatePlayerPosition() {
  ...
  if (surface !== undefined) {
    // If feet sank below surface, snap to stand on it
    ...
    // fallback (no blocks under): clamp to deepest layer
    const maxY = STONE_LAYERS * BLOCK_SIZE + characterYOffset;
    if (posY > maxY) {
      posY = maxY;
      velY = 0;
      grounded = true;
...
// === Transforms (First-Person: camera = character eyes) ===
function updateTransforms() {
  // Eye position: feetY + eyeHeight  == (posY - characterYOffset) + eyeHeight
  const eyeY = (posY - characterYOffset) + eyeHeight;
  const camX = posX;
  const camY = eyeY;
  const camZ = posZ;
  ...
  // First-person: no distance offset — place camera at the eyes
  const sceneTransform = `
    rotateX(${pitch}deg)
    rotateY(${yaw}deg)
    translate3d(${-camX}px, ${-camY}px, ${-camZ}px)
  `;
  // Keep the player model placed correctly (useful if you ever switch to 3rd person)
...
createCharacter();
console.log('World generated. Starting posY:', posY, 'feetY should be 0:', posY - characterYOffset);
animate();
