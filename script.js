...
// === Player state ===
let posX = 0;
let posY = groundY + characterYOffset; // place feet at surface level initially
let posZ = 0;
let yaw = 0, pitch = 0;

// character offset (distance from posY to model draw pos)
...
// === Ore vein generator (compact random walk) ===
function generateVein(startGX, startGY, startGZ, size, type) {
  ...
  while (stack.length > 0 && placed.length < size) {
    // Randomly pick a position from stack
    ...
    // Add neighbors to stack, bounded and with 90% chance to keep compactness
    const neighbors = [
    ...
    for (const n of neighbors) {
      if (
        ...
        const nk = keyAt(n.x,n.y,n.z);
        if (!visited.has(nk) && Math.random() < 0.9) {
          stack.push(n);
        }
      }
    }
...
// === Full multi-layer terrain generation with ore veins ===
function generateMultiLayerWorld() {
  ...
  for (let gx = 0; gx < CHUNK_SIZE_X; gx++) {
    for (let gz = 0; gz < CHUNK_SIZE_Z; gz++) {
      const dirtLayers = 2 + Math.floor(Math.random() * 2); // 2 or 3 dirt layers
      // layer 0 = grass
      ...
      // stone layers dirtLayers+1 .. STONE_LAYERS-1
  ...
  // Create DOM blocks for only exposed faces
  let createdCount = 0;
  for (const [k, type] of worldData.entries()) {
    const [gx, gy, gz] = k.split(',').map(Number);
    const exposed = getExposedFacesFor(gx, gy, gz);
    if (exposed.length === 0) continue;
    const blockEl = createBlockElement(gx, gy, gz, type, exposed);
    world.appendChild(blockEl);
    createdCount++;
  }

  console.log('generateMultiLayerWorld: blocks in worldData:', worldData.size, 'DOM blocks created:', createdCount);
}
...
function updatePlayerPosition() {
  ...
  // Collision with terrain blocks under player
  ...
  if (surface !== undefined) {
    ...
    // fallback: clamp to bottom of world if no blocks found
...
// === Update transforms (camera & player)
